...
class Marital
{
    ...
    public static function create()
    {
        ...
        {
            $statement = Self::$conn->prepare("INSERT INTO marital (title) VALUES (?)");
    ...
    public static function read()
    {
        if (!is_null(Self::$conn))
        {
            $statement = Self::$conn->prepare("SELECT id, title FROM marital
                WHERE id = ?
            ");
    ...
    public static function update()
    {
        if (!is_null(Self::$conn))
        {
            $statement = Self::$conn->prepare("UPDATE marital
                SET title = ?
                WHERE id = ?");
    ...
    public static function delete()
    {
        if (!is_null(Self::$conn))
        {
            $statement = Self::$conn->prepare("DELETE FROM marital WHERE id = ?");
            $statement->bind_param('i',
                Self::$params['id']
            );
